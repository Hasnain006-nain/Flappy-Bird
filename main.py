...
def welcomeScreen():
    """
    Show welcome images on the cover page
    """
    cover_image = pygame.image.load('message.jpg').convert()
    cover_image = pygame.transform.scale(cover_image, (SCREENWIDTH, SCREENHEIGHT))
    ...
    while True:
        for event in pygame.event.get():
            # if click cross button, close game
            if event.type == QUIT or (event.type == KEYDOWN and event.key == K_ESCAPE):
                pygame.quit()
                sys.exit()
            # press space or up key, start the game for them
...
if __name__ == '__main__':
    # main point where game starts
    pygame.init()  # initialize all pygame's modules
    FPSCLOCK = pygame.time.Clock()
    pygame.display.set_caption('Flappy bird by Hasnain Haider')
    ...
    GAME_SPRITES['message'] = pygame.image.load('message1.png').convert_alpha()
    ...
    while True:
        welcomeScreen()  # Shows the welcome screen to the user until he presses a button
